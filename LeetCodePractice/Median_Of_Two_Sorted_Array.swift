...
class Median_Of_Two_Sorted_Array: Solution {
    ...
    func findMedianSortedArrays(nums1: [Int], _ nums2: [Int]) -> Double {
        ...
        combinList = heap_sort_max(combinList, len: combinList.count)
        var mediaIndex = 0
...
//        print("\(combinList), Result: \(median)")
    ...
    func findMedianSortedArrays_right(nums1: [Int], _ nums2: [Int]) -> Double {
        var oneVal = nums1.first
        var twoVal = nums2.first
        var combinList = [Int]()
        var oneIndex = 0
        var twoIndex = 0
        func indexWithInsertWithNumber(num: Int, list: [Int]) -> Int {
            var objIndex = list.count - 1
            var index = -1
            while objIndex > 0 {
                let guardVal = list[objIndex]
                if num < guardVal {
                    index = list.indexOf(guardVal)!
                }
                else {
                    break
                }
        ...
        func insertVal(num: Int, index: Int) -> Void {
            if index > -1 {
                combinList.insert(num, atIndex: index)
            }
            else {
                combinList.append(num)
            }
        ...
        return caculateMedian(combinList)
    }
    ...
    func max_heapity(list: [Int], index: Int, len: Int) -> [Int] {
        var buildList = list
        let l = index * 2 + 1
        let r = l + 1
        var largest = index
    ...
    func build_heap_min(list: [Int], len: Int) -> [Int] {
        var buildList = list
        let maxIndex = len / 2 - 1
        for num in (0...maxIndex).reverse() {
    ...
    func min_heapity(list: [Int], index: Int, len: Int) -> [Int] {
        var buildList = list
        let l = index * 2 + 1
        let r = l + 1
        var min = index
    ...
    func head_sort_min(list: [Int], len: Int) -> [Int] {
